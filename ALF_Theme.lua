...
-- that is the deliberate trade for readability.
AT.uiScale = 1
AT.windows = AT.windows or {}

function AT.WinScale()
    return AT.Px(UIParent) * (AT.uiScale or 1)
end
...
-- (shut branch). :SetColor(c) tints both bars. Mouse-transparent.
...
function AT.LayoutPage(pg)
    ...
    for _, sec in ipairs(pg._sections) do
        ...
        if sec.visibleFn and not sec.visibleFn() then
            ...
            if col then
                -- an unresolved box (first pass, width still 0) must not
                -- clamp the column to nothing - the "label vanished, lone
                -- checkbox at the left edge" bug. Skip the cap until real.
